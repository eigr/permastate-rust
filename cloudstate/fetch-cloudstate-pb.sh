...
function fetch() {
  local path=$1
  local tag=$2
  mkdir -p proto/$(dirname $path)
  curl -o proto/${path} https://raw.githubusercontent.com/cloudstateio/cloudstate/${tag}/protocols/${path}
  #sed 's/^option java_package.*/option go_package = "${go_package}";/' proto/${path}
}
